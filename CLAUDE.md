...
- **Linear 연동**: 작업 시 해당 Linear 이슈의 상태를 동기화하고, 플랜 작성 시 코멘트로 추가한다.

### 커밋 단위
- **관심사별 분리**: 하나의 커밋은 하나의 관심사만 담는다. 기능 추가와 리팩토링이 섞이면 별도 커밋으로 분리한다.
- 예: 훅 추가 → 컴포넌트에 적용 → 관심사 분리 리팩토링 → 각각 별도 커밋
