...
export function FeedbackFormCreate() {
  ...
  const imageForm = useImageForm({ maxImages: MAX_FEEDBACK_IMAGES });
  ...
  const isDirty = content.length > 0 || imageForm.hasImageChanges;

  function handleSubmit() {
    if (content.length < MIN_FEEDBACK_CONTENT_LENGTH) return;
    mutate({ category, content, files: imageForm.selectedFiles });
  }
  ...
  return (
    <>
      <SubHeader title="피드백 작성" onBack={handleBack} />
